...
select * from orders-Data order by sales;
...
update orders_Data set city = null where order_id in ('US-2019-118983','CA-2020-152156')


select * from orders_data
where city is null

select count(*), count(order_id),count(city) as no_of_records
,count('Ankit'), count(distinct category) as dist_category, count(distinct city)
from orders_data
...
## from > where > group by> hiving> select > order by > limit
...
## from > joins > where > group by> hiving> select > order by > limit
...
else return_reason end as new_return_reason
from returns_data




## when profit< 0 -> loss, profit > 50 them low profit, <100 high profit, >100 very high profit
...
week(order_date) as order_mnt
from orders_data

select order_id,order_date,
QUARTER(order_date) as order_qtr
from orders_data


select order_id,order_date,ship_date
,TIMESTAMPDIFF(day,order_date,ship_date) as lead_days
from orders_data
